...
class SkylangDefinitionProvider {
    ...
    async provideDefinition(document, position, _token) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const typedMatch = line.match(new RegExp(`(?:(?:I|D|B|C|S)\\s+\\b${word}\\b|\\b${word}\\b\\s+(?:L|T|DICT|SET|SL|I|D|B|C|S))`));
            if (typedMatch) {
                const col = rawLine.indexOf(word);
                return new vscode.Location(document.uri, new vscode.Position(i, col));
            }
...
exports.SkylangDefinitionProvider = SkylangDefinitionProvider;
